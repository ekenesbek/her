...
import type { WebGoalState, WebPageLink } from "./types";
...
export function buildWebMcpRuntimeContext(userId: string, autoRecording: boolean) {
  ...
  const commonMemory = clipWebMemoryForPrompt(extractTimestampedMemoryEntries(readOrInitCommonWebMemory(userId)) || "(none yet)");
  const lines = [
